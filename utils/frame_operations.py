...
def resize(images, size=(120, 120)):
    ...
    for image in images:
        ...
        if image.shape < size:  # tuples are compared position by position
...
def normalize_intensity(images):
    ...
    for image in images:
        ...
        if is_color:
            image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        images_norm.append(cv2.equalizeHist(image))     # spread out intensity with approximately equal distribution
    return images_norm
